...
public:
    ...
    bool addKeyPose()
    {
        ...
        if (shared_data_->cache_unfiltered_key_poses.empty())
        {
            ...
            shared_data_->cache_unfiltered_cloud_history.push_back(measure_group_.lidar_org);
        }
...
public:
    ...
    void extractGround(size_t idx)
    {
        ...
        // 2. 有效地面点提取（类 ValidGroundFilter）
        pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_xyz(new pcl::PointCloud<pcl::PointXYZ>);
        for (const auto& pt : cloud_base->points)
        {
            if (pt.x >= 0.0 && pt.x <= 10.0 && pt.y >= -10.0 && pt.y <= 10.0 && pt.z <= 1.0)
                cloud_xyz->emplace_back(pt.x, pt.y, pt.z);
        }
    ...
    #pragma omp parallel for
        for (int i = 0; i < static_cast<int>(cloud_xyz->size()); ++i)
        {
            ...
            if (kdtree.radiusSearch(p, radius, indices, distances) > 0)
            {
                int valid_angle_count = 0, total_count = 0;
                for (size_t j = 0; j < indices.size(); ++j)
                {
                    const auto& p_d = cloud_xyz->points[indices[j]];
                    if (p_d.z > p.z)
                    {
                        float dx = p.x - p_d.x, dy = p.y - p_d.y, dz = p.z - p_d.z;
                        float alpha = atan2(-dz, std::sqrt(dx * dx + dy * dy));
                        if (p.z > 0.0){
                            valid_point = false;
                            break;
                        }
                        if (alpha > theta && -dz > 0.2)
                        {
                            valid_point = false;
                            break;
                        }
                        if (alpha <= theta / 3) valid_angle_count++;
                        if (-dz > 0.16) total_count++;
                    }
                }
                if (total_count > 0 && valid_angle_count < total_count / 2)
                    valid_point = false;

                if (valid_point)
                {
    #pragma omp critical
                    ground_candidates->points.push_back(p);
                }
            }
        }

        // 3. 统计滤波
        pcl::PointCloud<pcl::PointXYZ> M_filtered;
        pcl::StatisticalOutlierRemoval<pcl::PointXYZ> sor;
        sor.setInputCloud(ground_candidates);
        sor.setMeanK(30);
        sor.setStddevMulThresh(1.0);
        sor.filter(M_filtered);

        // 4. 转换为 fastlio 点类型
        pcl::PointCloud<fastlio::PointType>::Ptr ground_lidar(new pcl::PointCloud<fastlio::PointType>);
        for (const auto& pt : M_filtered)
        {
            fastlio::PointType p;
            p.x = pt.x;
            p.y = pt.y;
            p.z = pt.z;
            ground_lidar->points.push_back(p);
        }

        // 5. 转换回雷达坐标系
        Eigen::Matrix4f base2lidar = lidar2base_.inverse();
        pcl::transformPointCloud(*ground_lidar, *ground_lidar, base2lidar);
...
int main(int argc, char **argv)
{
    ...
    g_map_path = "/home/nvidia/Go2Nav2D/src/FASTLIO2_SAM_LC/PCD/map.pcd";
    g_ground_map_path = "/home/nvidia/Go2Nav2D/src/FASTLIO2_SAM_LC/PCD/ground_map.pcd";
    g_keyposes_path = "/home/nvidia/Go2Nav2D/src/FASTLIO2_SAM_LC/path/key_poses.txt";
    map_builder.run();
