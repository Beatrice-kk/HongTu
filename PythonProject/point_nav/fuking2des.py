...
# -*- coding: utf-8 -*-
'''
哈哈哈哈 没想到吧  你们几十万买的机器人  是我这个弱智实习生写的代码  哈哈哈哈哈哈
我叫cwk  from ZJUT，CHINA  
有问题也不能找我了哦
我也马上跑路拉啦啦啦啦啦啦啦
                     _ooOoo_
                    o88888888o
...
class SimpleNavWaypointPlayer:
    ...
    def arrival_callback(self, msg):
        ...
        if msg.data == "arrived":
            ...
            if self.current_waypoint_index == len(self.waypoints) - 1:
                # 到达最后一个航点，直接运行robust_navigation脚本
                rospy.loginfo("到达最后一个航点，启动robust_navigation脚本...")
                self._run_robust_navigation_script()
            else:
                self._handle_waypoint_arrival()
   ...
   #              self.force_move_to_next_waypoint()
    ...
    def force_move_to_next_waypoint(self):
        ...
        with self._lock:
            ...
            if self.current_waypoint_index < len(self.waypoints):
                ...
                # 所有路径点已完成，直接运行robust_navigation脚本
                rospy.loginfo("所有路径点已完成，启动robust_navigation脚本...")
                self._run_robust_navigation_script()
    ...
    def force_return_to_backstage(self):
        """强制返回后台点 - 直接运行robust_navigation_to_backstage.py"""
        with self._lock:
            ...
            # 直接运行robust_navigation_to_backstage.py脚本
            rospy.logwarn("启动robust_navigation_to_backstage.py脚本...")
            self._run_robust_navigation_script()

    def _run_robust_navigation_script(self):
        """运行robust_navigation_to_backstage.py脚本"""
        try:
            import subprocess
            import os
            ...
            if not os.path.exists(script_path):
                rospy.logerr(f"robust_navigation脚本不存在: {script_path}")
                return
            
            rospy.loginfo(f"运行robust_navigation脚本: {script_path}")
            ...
            rospy.loginfo("robust_navigation脚本已启动，等待完成...")
            
            # 等待脚本完成
            stdout, stderr = process.communicate()
            
            if process.returncode == 0:
                rospy.loginfo("robust_navigation脚本执行成功")
                rospy.loginfo(f"脚本输出: {stdout}")
            else:
                rospy.logwarn(f"robust_navigation脚本执行失败，返回码: {process.returncode}")
                rospy.logwarn(f"错误信息: {stderr}")
            
            # 脚本完成后，关闭当前程序
            rospy.loginfo("robust_navigation脚本完成，关闭当前程序")
            self._final_shutdown()
            
        except Exception as e:
            rospy.logerr(f"运行robust_navigation脚本失败: {e}")
            # 如果脚本运行失败，直接关闭程序
            self._final_shutdown()
