...
class NavWaypointPlayer:
    def __init__(self, backstage_pos, stage_entry_pos, dance_type, dance_choreography, threshold=0.5):
        ...
        self._progress_eps = 0.2  # 10 秒内至少前进 0.2m，否则认为规划/到达失败
    ...
    def _try_find_reachable_nearby(self, target_xytheta, current_xy):
        """
        尝试在目标附近搜索可达点。优先使用 make_plan 检查路径；否则基于距离退避。
        返回 (x,y,theta) 或 None
        """
    ...
    def feedback_callback(self, msg):
        ...
        # 卡滞/不可达检测：10s 内无明显前进，则在目标附近搜索可达点并重发目标
        now = rospy.Time.now()
        if now - self._last_progress_check_time > self._stuck_timeout and not self.reached_final and not self.dance_in_progress:
            ...
            if not progressed:
                rospy.logwarn("[规划] 目标可能不可达/无进展，尝试在附近搜寻可达点...")
                cur_xy = (current_pose.position.x, current_pose.position.y)
                new_goal = self._try_find_reachable_nearby(self.waypoints[self.current_waypoint_index], cur_xy)
                if new_goal is not None:
                    self.waypoints[self.current_waypoint_index] = new_goal
                    nx, ny, nth = new_goal
...
if __name__ == "__main__":
    ...
    # 定义固定位置
    backstage_pos = (0, 0, 0)  # 后台位置
    stage_entry_pos = (0, 0, 0)  # 舞台入口位置
