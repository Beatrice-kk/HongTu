...
class G1ActionPlayer:
    def __init__(self, action_dir="/home/unitree/unitree_sdk2_python/example/g1/high_level/action"):
        """
        初始化G1动作播放器
        ...
        # 预设TTS文本配置
        self.tts_presets = {
            'A': "亲爱的游客朋友们，大家好！欢迎来到国家5A级旅游景区——沙家浜！这里不仅是一座风光秀美的江南水乡，更是一方承载着“芦荡火种、鱼水情深”红色记忆的圣地。在这里，您可以走进沙家浜革命历史纪念馆，聆听那段可歌可泣的英雄故事，感受新四军与人民群众并肩作战的烽火岁月；也可以漫步芦苇荡间，乘一叶轻舟穿梭于碧波芦海，享受水乡的宁静野趣，领略独特的湿地风情；还可以来到横泾老街，漫步于上世纪三四十年代风貌的街巷，观看精彩的民俗表演，品尝地道的水乡美食，沉浸式体验淳朴悠然的江南民俗。沙家浜，是一幅自然与人文交织的画卷，更是一段值得用心感受的历史。愿您在这里度过一段充实而美好的时光！",
        ...
        # 控制参数
        self.base_kp_waist = 100.0
        self.base_kp_arm = 60.0

        self.base_kd_waist = 5.0
        self.base_kd_arm = 3.0
    ...
    def _send_interpolated_frame(self, smooth_ratio, target_idx):
        ...
        start_q = self.current_pose
        end_q = target_q
        # 使用平衡的平滑插值函数，在平滑度和响应速度之间取得平衡
        smooth_ratio = smooth_ratio * smooth_ratio * (3 - 2 * smooth_ratio)  # 标准平滑步进插值
        interp_q = (1 - smooth_ratio) * start_q + smooth_ratio * end_q
        # 优化控制参数，在平滑度和力度之间取得平衡
        kp_scale = 0.7 + 0.3 * smooth_ratio  # 保持一定的刚度，同时允许平滑过渡
        kd_scale = 0.8 + 0.2 * smooth_ratio  # 适度的阻尼以减少抖动
    ...
    def load_action(self):
        # 此函数已不再需要，因为我们在load_actions中处理了所有动作加载
        pass
    ...
    def _send_pose(self, q, dq=None, kp_scale=1.0, kd_scale=1.0):
        ...
        # 在动作播放期间提高发送频率到20ms一次 (~50Hz)，其他时候保持30ms
        if self.state in ["ramp_in", "playing"]:
            send_interval = 0.02  # 动作播放期间50Hz
        else:
            send_interval = 0.03  # 其他时候33Hz
        ...
        if self.state in ["ramp_in", "playing"]:
            # 动作播放期间使用更适合的控制参数
            kp_waist = self.base_kp_waist * kp_scale
            kp_arm = self.base_kp_arm * kp_scale
            kd_waist = self.base_kd_waist * kd_scale
            kd_arm = self.base_kd_arm * kd_scale
        else:
            # 其他状态保持平滑控制
            kp_waist = self.base_kp_waist * kp_scale * 0.8  # 降低腰部Kp值
            kp_arm = self.base_kp_arm * kp_scale * 0.7     # 降低手臂Kp值
            kd_waist = self.base_kd_waist * kd_scale * 1.2  # 增加腰部Kd值
            kd_arm = self.base_kd_arm * kd_scale * 1.3      # 增加手臂Kd值
    ...
    def update(self):
        ...
        # 限制update调用频率为50ms一次，平衡响应速度和CPU使用率
        ...
        if self.state == "ramp_in":
            ...
            self._send_interpolated_frame(smooth_ratio, target_idx=0)
            if ratio >= 1.0:
                self.state = "playing"
        ...
        if self.state == "move_to_initial":
            ...
            # 使用更平滑的插值函数
            smooth_ratio = ratio * ratio * (3 - 2 * ratio)  # 平滑步进插值
            ...
            #print(f"🔄 正在回到初始姿态: ratio={ratio:.2f}")
            #print(f"   起始位置: {start_q[:3]}")
            #print(f"   目标位置: {target_q[:3]}")
            #print(f"   当前插值位置: {interp_q[:3]}")

            # 发送插值位置命令，使用优化的控制参数
            self._send_pose(interp_q, dq=np.zeros(15), kp_scale=0.4, kd_scale=1.2)
            ...
            if ratio >= 1.0:
                ...
                # 发送最终位置命令
                self._send_pose(target_q, dq=np.zeros(15), kp_scale=0.4, kd_scale=1.0)
                # 设置状态为停止
                self.state = "stopped"
    ...
    def update_low_frequency(self):
        ...
        # 在停止状态下，以较低频率发送保持初始姿态的命令，防止关节松弛
        if self.state == "stopped":
...
class G1OdometryProcessor:
    ...
    def __init__(self):
        ...
        初始化里程计数据处理器
        """
        self.current_position = [0.0, 0.0, 0.0]  # x, y, z位置
        self.current_orientation = [0.0, 0.0, 0.0]  # roll, pitch, yaw欧拉角
    ...
    def odometry_callback(self, msg):
        ...
        try:
            with self.lock:
                ...
                # 更新欧拉角信息
                self.current_orientation[0] = msg.imu_state.rpy[0]
                self.current_orientation[1] = msg.imu_state.rpy[1]
                self.current_orientation[2] = msg.imu_state.rpy[2]
    ...
    def get_position_offset(self):
        ...
        Returns:
            list: [x_offset, y_offset, z_offset]
        """
        with self.lock:
            if self.initial_position is None:
                return [0.0, 0.0, 0.0]
            return [
                self.current_position[0] - self.initial_position[0],
                self.current_position[1] - self.initial_position[1],
                self.current_position[2] - self.initial_position[2]
            ]
    ...
    def get_yaw_offset(self):
        ...
        with self.lock:
            if self.initial_orientation is None:
                return 0.0
            # 处理角度环绕问题
            yaw_diff = self.current_orientation[2] - self.initial_orientation[2]
            # 将角度规范化到[-π, π]范围
...
# 自动化任务执行器
# -------------------------------
class G1AutoTaskExecutor:
    """G1机器人自动化任务执行器"""
    
    def __init__(self, loco_client, odometry_processor):
        """
        初始化自动化任务执行器
        
        Args:
            loco_client: LocoClient实例
            odometry_processor: G1OdometryProcessor实例
        """
        self.loco_client = loco_client
        self.odometry_processor = odometry_processor
        self.is_executing = False
        self.last_start_up_pressed = False
        
    def move_forward_precise(self, distance=1.0, speed=0.3):
        """
        精确前进指定距离
        
        Args:
            distance: 前进距离(米)
            speed: 前进速度(m/s) - 建议范围: 0.2-0.8 m/s
        """
        print(f"📍 开始精确前进 {distance} 米，速度: {speed} m/s")
        
        # 重置位置参考点
        self.odometry_processor.reset_reference()
        
        # 开始移动
        self.loco_client.Move(speed, 0.0, 0.0, True)
        
        # 等待到达目标位置
        target_reached = False
        start_time = time.time()
        timeout = distance / speed * 2 + 5  # 超时时间 = 预期时间 * 2 + 5秒
        
        while not target_reached and (time.time() - start_time) < timeout:
            # 获取当前位置偏移
            position_offset = self.odometry_processor.get_position_offset()
            # 计算在机器人前进方向上的位移
            forward_displacement = position_offset[0]  # X方向位移
            
            print(f"   前进进度: {forward_displacement:.2f}m / {distance}m")
            
            if abs(forward_displacement) >= distance:
                target_reached = True
            else:
                time.sleep(0.1)
        ...
        if target_reached:
            print(f"✅ 精确前进完成，实际位移: {self.odometry_processor.get_position_offset()[0]:.2f}m")
        else:
            print(f"⚠️  前进超时，实际位移: {self.odometry_processor.get_position_offset()[0]:.2f}m")
            
        return target_reached
    
    def turn_precise(self, angle_degrees=90, speed=0.5):
        """
        精确旋转指定角度
        
        Args:
            angle_degrees: 旋转角度(度)，正值为左转，负值为右转
            speed: 旋转速度(rad/s) - 建议范围: 0.3-1.0 rad/s
        """
        angle_radians = math.radians(angle_degrees)
        print(f"📍 开始精确旋转 {angle_degrees} 度 ({angle_radians:.2f} 弧度)，速度: {speed} rad/s")
        ...
        if angle_radians > 0:
            # 左转
            self.loco_client.Move(0.0, 0.0, speed, True)
        else:
            # 右转
            self.loco_client.Move(0.0, 0.0, -speed, True)
        ...
        target_reached = False
        start_time = time.time()
        expected_time = abs(angle_radians) / speed
        timeout = expected_time * 2 + 3  # 超时时间 = 预期时间 * 2 + 3秒
        
        while not target_reached and (time.time() - start_time) < timeout:
            # 获取当前角度偏移
            yaw_offset = self.odometry_processor.get_yaw_offset()
            
            print(f"   旋转进度: {math.degrees(yaw_offset):.1f}° / {angle_degrees}°")
            
            if abs(yaw_offset) >= abs(angle_radians):
                target_reached = True
            else:
                time.sleep(0.1)
        ...
        if target_reached:
            print(f"✅ 精确旋转完成，实际角度: {math.degrees(self.odometry_processor.get_yaw_offset()):.1f}°")
        else:
            print(f"⚠️  旋转超时，实际角度: {math.degrees(self.odometry_processor.get_yaw_offset()):.1f}°")
            
        return target_reached
    
    def execute_forward_and_turn(self):
        """
        执行前进2米然后左转90度的任务
        """
        if self.is_executing:
            print("⚠️  任务已在执行中，请等待完成")
            return False
            
        try:
            self.is_executing = True
            print("🚀 开始执行自动化任务：前进2米并左转90度")
            ...
            if code == 0 and data:
                ...
                fsm_id = result.get("data", 0)
            else:
                print(f"❌ 获取机器人模式失败: {code}")
                self.is_executing = False
                return False
                
            if fsm_id not in [500, 501, 801]:  # 主运控模式
                print("⚠️  机器人未处于主运控模式，尝试切换到主运控模式")
                self.loco_client.Start()  # 切换到主运控
                time.sleep(2)  # 等待模式切换完成
                
            # 执行精确前进2米，速度0.4m/s（原为0.3m/s）
            forward_success = self.move_forward_precise(1.0, 0.3)
            time.sleep(1)
            
            # 执行精确左转90度，速度0.6rad/s（原为0.5rad/s）
            turn_success = self.turn_precise(90, 0.5)
            
            if forward_success and turn_success:
                print("✅ 自动化任务执行完成")
            else:
                print("⚠️  自动化任务执行完成，但可能存在精度问题")
            
            self.is_executing = False
            return True
            
        except Exception as e:
            print(f"❌ 执行自动化任务时出错: {e}")
            self.is_executing = False
            return False
    
    def handle_start_up_key(self, remote, player):
        """
        处理Start+Up按键组合
        
        Args:
            remote: 遥控器对象
            player: G1ActionPlayer实例
        """
        start_up_pressed = remote.Start and remote.Up
        
        # 检测Start+Up按键按下事件
        if start_up_pressed and not self.last_start_up_pressed and player.in_main_loco_mode:
            print("🎮 检测到 Start + Up 按键，准备执行自动化任务")
            
            # 只有在功能激活时才执行任务
            if player.function_activated:
                self.execute_forward_and_turn()
            else:
                print("🔒 功能未激活，请先按 F1 + Start 激活功能")
        
        self.last_start_up_pressed = start_up_pressed
...
class G1LocoModeChecker:
    ...
    def __init__(self, network_interface="enp6s0"):
        """
        初始化检测器
...
def main(return_remote=False):
   ...
   #  network_interface = sys.argv[1] if len(sys.argv) > 1 else "lo"
    ...
    action_dir = "/home/unitree/unitree_sdk2_python/example/g1/high_level/action"  # 默认动作目录
    ...
    def lowstate_callback(msg):
        try:
            ...
            if player.function_activated:
                ...
                # 处理Start+A/B/X/Y组合键，用于播放预设的TTS文本
                elif remote.get_combo_once('Start', 'A'):
                    print("🔊 检测到 Start + A，播放预设TTS文本A")
                    try:
                        # 添加防重复机制
                        ...
                        if current_time - player._last_tts_a_time > 1.0:  # 至少间隔1秒
                            # 播放预设文本A
                            player._play_tts_with_wait(player.tts_presets['A'], 0)
                            player._last_tts_a_time = current_time
                            print("✅ TTS文本A播放完成")
                    except Exception as e:
                        print(f"❌ 播放TTS文本A时出错: {e}")
                        
                elif remote.get_combo_once('Start', 'B'):
                    print("🔊 检测到 Start + B，播放预设TTS文本B")
                    try:
                        # 添加防重复机制
                        ...
                        if current_time - player._last_tts_b_time > 1.0:  # 至少间隔1秒
                            # 播放预设文本B
                            player._play_tts_with_wait(player.tts_presets['B'], 0)
                            player._last_tts_b_time = current_time
                            print("✅ TTS文本B播放完成")
                    except Exception as e:
                        print(f"❌ 播放TTS文本B时出错: {e}")
                        
                elif remote.get_combo_once('Start', 'X'):
                    print("🔊 检测到 Start + X，播放预设TTS文本C")
                    try:
                        # 添加防重复机制
                        ...
                        if current_time - player._last_tts_c_time > 1.0:  # 至少间隔1秒
                            # 播放预设文本C
                            player._play_tts_with_wait(player.tts_presets['C'], 0)
                            player._last_tts_c_time = current_time
                            print("✅ TTS文本C播放完成")
                    except Exception as e:
                        print(f"❌ 播放TTS文本C时出错: {e}")
                        
                elif remote.get_combo_once('Start', 'Y'):
                    print("🔊 检测到 Start + Y，播放预设TTS文本D")
                    try:
                        # 添加防重复机制
                        ...
                        if current_time - player._last_tts_d_time > 1.0:  # 至少间隔1秒
                            # 播放预设文本D
                            player._play_tts_with_wait(player.tts_presets['D'], 0)
                            player._last_tts_d_time = current_time
                            print("✅ TTS文本D播放完成")
                    except Exception as e:
                        print(f"❌ 播放TTS文本D时出错: {e}")
                else:
                    # 在功能激活状态下持续更新player状态
                    player.update()
                    # 添加小延迟以降低CPU使用率
                    time.sleep(0.05)  # 50ms延迟
            else:
                # 功能未激活时，如果正在播放动作，则立即停止
                if player.state != "stopped":
                    print("⚠️  功能未激活，正在停止当前动作...")
                    player.stop_play()
                
                # 功能未激活时，以较低频率更新player状态，减少CPU占用
                if not hasattr(player, '_last_update_time'):
                    player._last_update_time = 0
                
                # 降低更新频率到每500ms一次
                current_time = time.time()
                if current_time - player._last_update_time >= 0.5:
                    player.update_low_frequency()
                    player._last_update_time = current_time
                else:
                    # 添加小延迟以降低CPU使用率
